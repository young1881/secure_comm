...
使用ECDH密钥交换自动派生加密密钥，无需手动提供密钥和IV：
...
- `-x <kex>` : **密钥交换算法（推荐使用）**
  - `ecdhp256` - ECDH-P256（推荐）
  - `ecdhp384` - ECDH-P384
  - 使用此参数启用密钥交换，无需提供 `-k` 和 `-v`
- `-k <key>` : 加密密钥（十六进制字符串，仅在不使用 `-x` 时需要）
- `-v <iv>` : 初始化向量（十六进制字符串，仅在不使用 `-x` 时需要）
- `-K <pubkey>` : 签名公钥文件（PEM格式，必需）

**注意**：要么使用 `-x` 启用密钥交换，要么提供 `-k` 和 `-v` 使用手动密钥。两者不能同时使用。
...
### secure_server 和 secure_client 的行为
...
1. **服务器端行为**：
   ...
   - 客户端连接后，首先进行ECDH密钥交换：
     - 服务器生成ECDH密钥对
     - 接收客户端公钥，发送服务器公钥
     - 派生共享密钥，并从中派生对称加密密钥和IV
   - 进入循环等待接收 `PKT_TYPE_ENCRYPTED_SIGNED` 类型的数据包
   - 收到数据包后：
     - 先验证签名（验签）
     - 然后解密数据（解密）
     - 打印解密后的消息
...
**注意**：手动密钥模式需要手动提供密钥和IV，**不包含密钥交换流程**。推荐使用密钥交换模式（`-x` 参数）以获得更好的安全性。
...
./bin/secure_client -i 127.0.0.1 -p 8888 \
    ...
    -x ecdhp256 \
    -K private_key.pem \
    -m "Hello, this is a secure message with key exchange!"
```

### 示例4：手动密钥的完整加密通信（向后兼容）
